...
app.get('/', async (c) => {
  const html = await eta.renderAsync('index', { title: 'Parnuan Submission Assignment 1' })
...
app.post('/ui/confirm', async (c) => {
  ...
  const itemsHtml = items
    .map(
      (item, index) => `
        <div class="flex items-center justify-between p-3.5 bg-slate-50/80 hover:bg-slate-50 border-2 border-slate-200 rounded-xl transition-all">
